# MSFold

This release accompanies the manuscript *Sampling in structure-token space enables accurate prediction of multiple conformations*.

MSFold samples protein conformational ensembles using parallel tempering
with block-wise replica exchange, built on ESM3 structure tokens.

## Setup

### Step 1. Clone MSFold

Use the following commands to clone this repository and enter the project directory.

```bash
git clone git@github.com:findless/MSFold.git
cd MSFold
```

### Step 2. Clone ESM, switch to the required version, and complete ESM3 setup

MSFold depends on the official ESM3 codebase. Clone the ESM repository into `./esm`, switch it to `v3.1.1`, and then follow the official ESM instructions for any additional ESM3 setup and weight access.

```bash
git clone https://github.com/evolutionaryscale/esm.git esm
cd esm
git checkout v3.1.1
cd ..
```

Official ESM instructions:

- `https://github.com/evolutionaryscale/esm`

### Step 3. Create the environment and install ESM + MSFold

Create the conda environment, activate it, then install the checked-out `./esm` and MSFold.

```bash
conda env create -f environment.yml
conda activate msfold
pip install -e ./esm
pip install -e .
```

## Run MSFold sampling

### Step 4. Run the example sampling job

Use the example FASTA and default configuration to verify that the installation works.

```bash
python scripts/run_msfold_sample.py \
  --fasta examples/example.fasta \
  --config configs/default.yaml \
  --out outputs/example \
  --device cuda
```

### Step 5. Check the output files

After the run finishes, the following files should appear under `outputs/example/`:

- `samples.csv`
- `*.pdb`
- `config_used.json`
- `run_metadata.json`

## Rank samples by sequence log-likelihood

### Step 6. Rank the sampled structures

Run the following command to write the SLL ranking table.

```bash
python scripts/rank_sll.py \
  --sample_dir outputs/example \
  --out outputs/example/sll_ranking.csv
```

## Optional side-chain packing

The public MSFold release should be treated as producing backbone / structure-level
outputs. Although the ESM3 paper describes all-atom generation capability,
publicly usable ESM3 weights do not provide the side-chain generation capability
required for the original internal workflow, so this release should be treated as
backbone-only in practice.

In our own use, we packed side chains for MSFold-generated backbones with
FlowPacker. If you need packed side chains for structures generated by MSFold, you
can run FlowPacker or another side-chain packing tool as an external postprocess
step on the exported PDBs.

- Code: `https://gitlab.com/mjslee0921/flowpacker`
- Paper: Lee JS, Kim PM. *FlowPacker: protein side-chain packing with torsional flow matching*. Bioinformatics (2025). `10.1093/bioinformatics/btaf010`

Please follow the official FlowPacker instructions for installation and inference.

## Citation

```bibtex
@article{Wang2026.03.03.708411,
  author = {Wang, Zhiyuan and Yu, Yue and Yu, Chungong and Bu, Dongbo},
  title = {Sampling protein structural token space enables accurate prediction of multiple conformations},
  journal = {bioRxiv},
  year = {2026},
  doi = {10.64898/2026.03.03.708411},
  url = {https://www.biorxiv.org/content/early/2026/04/08/2026.03.03.708411}
}
```
